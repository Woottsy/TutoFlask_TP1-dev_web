...
def index():
    ...
    if len(request.args) == 0:
        return render_template(
            "index.html", title="R3.01 Dev Web avec Flask", name="Cricri"
        )
    else:
        param_name = request.args.get("name")
        return render_template(
            "index.html", title="R3.01 Dev Web avec Flask", name=param_name
        )
...
def contacts():
    return render_template(
        "contact.html",
        title="Dans les contacts des abysses",
        name="Cristophe",
        var1="pain",
    )
...
def getAuteurs():
    ...
    return render_template(
        "auteurs_list.html", title="R3.01 Dev Web avec Flask", auteurs=lesAuteurs
    )
...
def updateAuteur(idA):
    ...
    return render_template(
        "auteur_update.html", selectedAuteur=unAuteur, updateForm=unForm
    )
...
def saveAuteur():
    ...
    return render_template(
        "auteur_update.html", selectedAuteur=updatedAuteur, updateForm=unForm
    )
